...
# Create item-item sales matrix
item_sales_matrix = sales_df[['item_id', 'sales_normalized']].set_index('item_id')
sales_similarity = cosine_similarity(item_sales_matrix)
sales_similarity_df = pd.DataFrame(sales_similarity, index=sales_df['item_id'], columns=sales_df['item_id'])
...
combined_similarity = (alpha * content_similarity_df + 
                       ...
                       gamma * sales_similarity_df)

combined_similarity_df = pd.DataFrame(combined_similarity, index=items_df['item_id'], columns=items_df['item_id'])

# Function to get the similar items
def get_similar_items(item_id, similarity_matrix, top_n=5):
    similar_items = similarity_matrix[item_id].sort_values(ascending=False).head(top_n + 1)
    similar_items = similar_items.drop(item_id)  # Drop the item itself
...
def recommend_item():
    ...
    if item_id not in combined_similarity_df.index:
        return jsonify({'error': 'Item ID not found'}), 404

    similar_items = get_similar_items(item_id, combined_similarity_df)
    return jsonify(similar_items.to_dict())
